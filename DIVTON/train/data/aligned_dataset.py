...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        dir_PI = "_pascal"
        self.dir_PI = os.path.join(opt.dataroot, opt.phase + dir_PI)
        self.PI_paths = sorted(make_dataset(self.dir_PI))
    ...
    def __getitem__(self, index):        
        ...
        PI_path = self.PI_paths[index]
        PI = Image.open(PI_path).convert('L')
        PI_tensor = transform_A(PI) * 255.0
        ...
        if self.opt.isTrain:
            input_dict = { 'label': A_tensor, 'parsing_input': PI_tensor,'image': B_tensor, 'path': A_path, 'img_path': B_path ,'color_path': C_path,'color_un_path': C_un_path,
