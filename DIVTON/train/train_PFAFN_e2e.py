...
from models.networks import ResUnetGenerator, VGGLoss, save_checkpoint, load_checkpoint_parallel, ParsingNetwork
...
PF_warp_model = AFWM(opt, parsing_input_size+3)
PF_warp_model.train()
PF_warp_model.cuda()

PF_gen_model = ResUnetGenerator(7, 5, 5, ngf=64, norm_layer=nn.BatchNorm2d)
PF_gen_model.train()
PF_gen_model.cuda()

PB_warp_model = AFWM(opt, 45, skip=True)
PB_warp_model.eval()
PB_warp_model.cuda()
...
if not opt.load_pretrain:
    ...
    load_checkpoint_parallel(PF_gen_model, f"checkpoints/{opt.name}/PFAFN_gen_epoch_{opt.load_pretrain}.pth")
    load_checkpoint_parallel(PF_warp_model, f"checkpoints/{opt.name}/PFAFN_warp_epoch_{opt.load_pretrain}.pth")
  
    start_epoch = int(opt.load_pretrain)
...
if opt.isTrain and len(opt.gpu_ids):
    PF_warp_model = torch.nn.parallel.DistributedDataParallel(PF_warp_model, device_ids=[opt.local_rank], find_unused_parameters=True)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(train_loader):
        ...
        parse_p_tryon_un = parsing_model(p_tryon_un)
        parsing_shape = (size[0], parsing_input_size, size[2], size[3])
        parsing_warping_input = torch.cuda.FloatTensor(torch.Size(parsing_shape)).zero_()
        parsing_warping_input = parsing_warping_input.scatter_(1, parse_p_tryon_un, 1.0)

        pf_warp_concat = torch.cat((p_tryon_un.detach(), parsing_warping_input), 1)

        flow_out = PF_warp_model(pf_warp_concat, clothes.cuda(), pre_clothes_edge.cuda())
        ...
        parse_p_tryon = parsing_model(p_tryon, True)
        loss_parsing = F.cross_entropy(parse_p_tryon, data["parsing_input"].data.long().squeeze(1).cuda())
        ...
        gen_mask_loss_l1 = criterionL1(gen_mask, torch.FloatTensor((data['parsing_input'].cpu().numpy()!=0).astype(np.int32)).cuda())
        ...
        loss_all = 0.25 * loss_warp + loss_gen + opt.parsing_alpha * loss_parsing + gen_mask_loss_l1

        if opt.local_rank == 0:
            writer.add_scalar('loss_gen', loss_gen, step)
            writer.add_scalar('loss_parsing', loss_parsing, step)
        ...
        if step % 1000 == 0:
            if opt.local_rank == 0:
                ...
                gm = torch.cat([gen_mask, gen_mask, gen_mask], 1)
                i = p_tryon
                combine = torch.cat([a[0], b[0], c[0], d[0], e[0], f[0], g[0], h[0], gm[0], i[0]], 2).squeeze()
        ...
        if step % 100 == 0:
            if opt.local_rank == 0:
                print('{}:{}:[step-{}]--[loss-{:.6f}]--[loss-{:.6f}]--[loss-{:.6f}]--[ETA-{}]'.format(now, epoch_iter, step, loss_gen, loss_warp, loss_parsing  , eta))
