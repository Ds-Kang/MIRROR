...
class Vgg19(nn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.slice5[6] = nn.MaxPool2d(kernel_size=(2,1), stride=(2,1), padding=0, dilation=1, ceil_mode=False)
...
class ParsingNetwork(nn.Module):
    def __init__(self, opt):
        ...
        self.parse_model = resnet101(num_classes=7, pretrained=None)
        state_dict = torch.load(opt.parsing_checkpoint, map_location='cuda:{}'.format(opt.local_rank))['state_dict']
        ...
        self.parse_model.load_state_dict(new_state_dict)  

    def forward(self, frame, return_logits=False):
        parsing_input = F.interpolate(frame, size=(512,512), mode='bilinear', align_corners=True)        
        parse_out = self.parse_model(parsing_input)

        logits_result = F.interpolate(parse_out, size=(256,192), mode='bilinear', align_corners=True)
        if return_logits:
            return logits_result
        parsing_frame = torch.argmax(logits_result, dim=1).unsqueeze(1)

        return parsing_frame
...
def load_checkpoint_parallel(model, checkpoint_path):
    ...
    if not os.path.exists(checkpoint_path):
        print('No checkpoint!: ',checkpoint_path)
        return
